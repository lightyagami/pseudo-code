...
public:
    ...
    struct ClassInfo {
        ...
        std::unordered_map<std::string, bool> methodVisibility; // true if private
        int line = 0, col = 0;
    };
    ...
    const std::unordered_map<std::string, ClassInfo>& classTypes() const { return classTypes_; }
...
private:
    ...
    std::unordered_map<std::string, ClassInfo> classTypes_;
    ...
    bool assignable(const TypeInfo& to, const TypeInfo& from) const;
    const FieldDef* lookupField(const std::string& recordName, const std::string& fieldName);
    const ClassProperty* lookupClassProperty(const std::string& className, const std::string& fieldName, std::string* outDeclaringClass = nullptr) const;
    const FunctionSig* lookupClassMethod(const std::string& className, const std::string& methodName, bool* outIsPrivate = nullptr, std::string* outDeclaringClass = nullptr) const;
    void resolveType(TypeInfo& t);
